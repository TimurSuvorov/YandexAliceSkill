...
def hi_replies(user_id: str, session_id: str) -> dict:
        ...
        if int(allscores) != 0:
            rating_message += random.choice(
                [f'Ты набрал {decl_scores(allscores)} за всё время.\n',
                 ...
                 f'В твоей коп+илке {decl_scores(allscores)}.\n'
                 ]
            )
            ...
            rating_message += random.choice(
                [
                    ...
                    'Уверена, что ты можешь на б+ольшее. Прод+олжим игру? '
                 ]
                                            )
        # Если у пользователя ноль баллов
            ...
            rating_message += random.choice(
                ['Не густо. Начнём покорять вершины? ',
                 'Маловато будет. Прод+олжим игру, чтоб это исправить? ',
                 'Уверена, что ты можешь на б+ольшее. Прод+олжим игру, чтоб это исправить? '
                ]
            )
    ...
    response: dict = {
            ...
            'tts': f'{startsound}{hi_text} {rating_message}',
            'end_session': 'false'
    }
