...
def fucking_replies(command: str, session_state: dict) -> dict:
    ...
    if not session_state.get("question_dict"):
        response: dict = {
            ...
            'card': {
                'type': 'BigImage',
                'image_id': Image.OBSCENE.id,
                'title': 'Ай-яй-яй...',
                'description': remove_tts_symbols(f'{fucking_sentence} {letstart_sentence}')
            },
            'buttons': [
                {'title': 'Да', 'hide': 'true'},
                {'title': 'Нет', 'hide': 'true'}
